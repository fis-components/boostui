 ...
 */
    ...
    function color2Hex(str) {
        function toHex(n) {
            n = Math.max(Math.min(Math.floor(n), 255), 0) + 256;
...
;
    ...
    (function ($) {
        function dynamicLoad(callback) {
            var _doc = document.getElementsByTagName('head')[0], _script = document.createElement('script'), _loadScript = BLENDURL;
 ...
 */);
     ...
     */
         ...
         */
            _init: noop,
            destroy: function () {
                this._destroy();    // TODO 批量删除事件绑定好像不大好做 -_-!!
         ...
         */
            _trigger: function (type, originalEvent, data) {
                ...
                data = data || {};
                event = $.Event(type, { originalEvent: originalEvent });
 ...
 */
     ...
     */
            _randomID: function () {
                return ((1 + Math.random()) * 4096 | 0).toString(16);
            },
            /**
     * 创建UIX的Dialog
     ...
     */
            _createUIXDialog: function (blend) {
                ...
                if ((this.btnStatus & 1) > 0) {
                    var confirmItem = uixDialog.create({ text: confirmText });
                ...
                if ((this.btnStatus & 2) > 0) {
                    var cancelItem = uixDialog.create({ text: cancelText });
     ...
     */
            _createHTMLDialog: function () {
                ...
                if (this.renderType === 0) {
                    curEle = this.$el;
                    curEle.find('.' + NAMESPACE + 'dialog-footer a').addClass(NAMESPACE + 'dialog-btn');
                    outerEle = curEle;
     ...
     */
            _getDialogHtml: function () {
                var dom = '<div class="' + NAMESPACE + 'dialog-header">' + this.title + '</div>' + '<div class="' + NAMESPACE + 'dialog-body">' + this.content + '</div>' + '<div class="' + NAMESPACE + 'dialog-footer">' + '<a href="javascript:void(0);" class="' + this.confirmClass + ' ' + NAMESPACE + 'dialog-confirm ' + NAMESPACE + 'dialog-btn">' + this.confirmText + '</a>' + '<a href="javascript:void(0);" class="' + this.cancelClass + ' ' + NAMESPACE + 'dialog-cancel ' + NAMESPACE + 'dialog-btn">' + this.cancelText + '</a>' + '</div>';
                this.$el.append(dom);
                return this.$el;
            },
            /**
     ...
     */
            setPosition: function () {
                var top = typeof this.top === 'undefined' ? window.innerHeight / 2 - this.$el[0].clientHeight / 2 : parseInt(this.top, 10);
 ...
 */
     ...
     */
            _initUIXGallery: function (blend) {
                var uixGallery = blend.create('gallery', { images: this.options.data });
                return uixGallery;
            },
            /**
     * 创建遮罩mask
    ...
    */
     ...
     */
            _slideTo: function (dataIndex) {
                ...
                if (n !== 0) {
                    ...
                    }, 200);    // this.onslidechange && this.onslidechange(this.slideIndex);
     ...
     */
            _moveHandler: function (evt) {
                if (this.isMoving) {
                    ...
                    var res = this.zoomMoveHandler ? this.zoomMoveHandler(evt) : false;
                    // zoom  事件
                    // var res = false;
     ...
     */
            _endHandler: function (evt) {
                ...
                var res = this.zoomEndHandler ? this.zoomEndHandler(evt) : false;
                // zoom  事件
                // var res = false;
        ...
        $.widget('blend.header', {
            ...
            _initUIXComponent: function (blend) {
                ...
                uixTitle = blend.create('title', {
                    text: $titleItems.text()    // TODO 支持Image
                });
                uixTitle.setStyle({
        ...
        function __genItemIterator(cb) {
            return function (i, item) {
                ...
                if ($item.hasClass(ACTION_BACK_CLASS)) {
                    retObj.action = { operator: 'back' };
                } else if (nodeName && nodeName.toUpperCase() === 'A') {
                    retObj.action = { url: item.href };
                }
 ...
 */
     ...
     */
            options: {
                ...
                animate: true,
                // 动画的开关
                itemClass: NAMESPACE + 'list-item',
                // 滑动的element的class
                animateClass: NAMESPACE + 'list-animation',
                // 动画实现的class
                itemContentClass: NAMESPACE + 'list-item-content',
                // 列表主体element的class
 ...
 */
     ...
     */
         ...
         */
                ...
                that._fnAutoSwipe();
                this._initEvent();    // this._initView();
            },
            /**
     * 初始化事件绑定
     * @private
     ...
     */
            _fnMove: function () {
                ...
                that._setDotActive();    // callback(_index);
            },
            /**
     * 滑动
     * @private
 ...
 */
     ...
     */
            _initUIXComponent: function (blend) {
                var uixTab, me = this, $el = this.element, $tabItem = $el.find(this._itemSelector);
                /*创建一个UIXtab*/
     ...
     */
            _generateItem: function (callback, uixTab) {
                return function (index, _item) {
                    var $item = $(_item), blendItem, itemConf = {
                            text: $item.text(),
                            href: $item.data('href')
                        }, itemTab;
                ...
                if (this.defaultIcon) {
                    for (var i = 0, len = $items.length; i < len; i++) {
                        ...
                        if (item.getElementsByTagName('ul').length > 0) {
                            $(item).find('span').addClass(NAMESPACE + 'topnav-arrow').addClass(NAMESPACE + 'topnav-downarrow');
                        }
                    }
                }
                /**
            ...
            _bindEvent: function () {
                ...
                for (var i = 0, len = $items.length; i < len; i++) {
                    ...
                    $item.on('tap, click', function (e) {
                        ...
                        $items.not(i).find('span').removeClass(NAMESPACE + 'topnav-uparrow').addClass(NAMESPACE + 'topnav-downarrow');
                        if (isActive) {
            ...
            $('[data-blend-widget]').each(function (i, elem) {
                ...
                for (index = 0; index < widgetNameLen; index++) {
                    ...
                    if ($.widget.has(name)) {
                        ...
                        // TODO error report
                        throw new Error('Unknow blend widget "' + name + '"');    // console.error('Unknow blend widget \"' + name + '\"');
                    }
                }
            });
        });
